...
def get_signal(ticker):
    try:
        ...
        conditions_up = (
            latest_rsi < 30
            and latest_close > latest_ema50
            and stochastic_signal == "bullish"
        )
        conditions_down = (
            latest_rsi > 70
            and latest_close < latest_ema50
            and stochastic_signal == "bearish"
        )
...
def start(update: Update, context: CallbackContext):
    update.message.reply_text(
        "Привіт! Я твій бот для автоматичних сигналів. Вибери валютні пари командою /pairs"
    )
...
def pair_selected(update: Update, context: CallbackContext):
    ...
    if text in pairs_list:
        ...
        update.message.reply_text("Невідома пара. Спробуй ще раз командою /pairs.")
...
def stop(update: Update, context: CallbackContext):
    update.message.reply_text("Зупинено.")
